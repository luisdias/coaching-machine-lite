...
class SetupConfig extends SetupConfigHTML{
    ...
    private function MakeFile(){
        $file1 = DIRECTORY_SEPARATOR . $this->file1;
        $file2 = DIRECTORY_SEPARATOR . $this->file2;

        if ($this->CheckConnection($_POST) == true){
            $this->WriteUTF8File(realpath(__DIR__) . $file1, $this->ReplaceASCIIFwrite($this->FileGenerate($_POST,1)));
            $this->WriteUTF8File(realpath(__DIR__) . $file2, $this->ReplaceASCIIFwrite($this->FileGenerate($_POST,2)));

            $path  = '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR;

            copy($this->file1, $path . 'database.php');
            copy($this->file2, $path . 'coaching_machine_settings.php');

            chmod(realpath(__DIR__) . $file1, 0666);
            chmod(realpath(__DIR__) . $file2, 0666);

            if (file_exists(realpath(__DIR__) . $file1) && file_exists(realpath(__DIR__) . $file2)) 
